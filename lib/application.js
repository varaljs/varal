...
class Application {
    ...
    error(err) {
        return this.serverError(err, this);
    }
        ...
        if (result.path_match === true) {
            ...
            if (result.method_match === true) {
                ...
                if (typeof result.callback === 'function')
                    ...
                    let controller = require(this.rootPath + helper.pathFormat(this.controllerPath) + helper.pathFormat(action[0]));
    ...
    render(view, data) {
        const template = fs.readFileSync(this.rootPath + '/' + this.viewPath + '/' + view + '.hbs', 'utf-8');
